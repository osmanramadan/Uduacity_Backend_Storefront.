...
describe("########################## users end points #####################", () => {
    describe("main end point:", () => {
        it("/ :", async () => {
            const response = await request.get('/users').set(auth);
    ...
    describe("sub of main end point /:", () => {
        ...
        it("/:userid/purchases <user purchases>", async () => {
            const response = await request.get('/users/3/purchases').set(auth);
            expect(response.status).toBe(200);
        });
        ...
        it("/update <update user>", async () => {
            const response = await request.put('/users/update').set(auth);
            expect(response.status).toBe(200);
        });
        it("/del <del user>", async () => {
            const response = await request.delete('/users/delete/7').set(auth);
            expect(response.status).toBe(200);
        });
